...
## Фаза 5: Интеграция и Реактивность — ЗАВЕРШЕНО ✅
...
- [x] 5.2. Реализация логики Watchdog в Uart_Task.cpp:
    - [x] Контроль `millis() - lastUpdate`.
    - [x] Автоматическая отправка `0x10 (PING)` при превышении порога 65с.
- [x] 5.3. Оптимизация Uart_Task для минимального потребления CPU в ожидании данных (`vTaskDelay(10)`).
- [x] 5.4. Сквозной тест: "Живые" данные с ADC STM32 -> Отображение в UI.
...
- [ ] **6.1. Сетевой уровень (Industrial Transition):**
    ...
    - [ ] **mDNS Discovery:** Доступ по `http://heater.local`.
- [ ] **6.2. Надежность и Хранение:**
    - [ ] **NVS Persistence:** Сохранение уставки температуры в энергонезависимую память.
    - [ ] **OTA Updates:** Внедрение системы беспроводного обновления (ArduinoOTA).
- [ ] **6.3. Интерфейс:**
    - [ ] Переход на **WebSockets** для мгновенной обратной связи.
    - [ ] Обработка ошибки `-999` (SensErr) с явным баннером в UI.
