...
- [x] **3.1. Uart_Task (Core 1)**
    ...
    - [x] **Отправка команд из `Cmd_Srv`**: Полная интеграция очереди.
- [x] **3.2. Network_Task (Core 0)**
    - [x] Запуск WiFi (в режиме Pure AP Mode).
    - [x] **WebServer (MVP):** Реализация на базе стандартной `WebServer.h`.
    - [x] Реализация эндпоинта GET `/api/status`.
    - [x] **Эндпоинт управления `/api/set_temp`**: Прямая передача команд пользователем.
